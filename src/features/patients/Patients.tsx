...
export const Patients = () => {
  ...
  const { patientViewMode: viewMode, setPatientViewMode: setViewMode } =
    useUIStore();
          ...
          {paginatedPatients.map((patient) => (
            <div key={patient.id} className={styles.card}>
              ...
              <div className={styles.details}>
                ...
                <div>
                  <span
                    className={`${styles.status} ${getStatusClass(patient.status)}`}
                  >
                    {patient.status}
                  </span>
                </div>
      ...
      {totalPages > 0 && (
        <div className={styles.pagination}>
          <span className={styles.pageInfo}>
            ...
            {Math.min(startIndex + ITEMS_PER_PAGE, filteredPatients.length)} of{" "}
